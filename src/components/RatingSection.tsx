        ...
        <div className="rounded-2xl border border-line bg-white p-2">
          {leaderboard.map((entry) => (
            <div key={entry.rank} className="mb-1 last:mb-0">
              <div
                className={`grid grid-cols-[28px_1fr_60px] items-center gap-3 rounded-[10px] px-3.5 py-3 ${
                  entry.rank === 1 ? "bg-canvas-2" : ""
                }`}
              >
                <span
                  className={`font-mono text-[13px] font-bold ${
                    entry.rank === 1 ? "text-primary-deep" : "text-ink-soft"
                  }`}
                >
                ...
                <span className="text-right font-mono text-[13px] text-ink-soft">
                  {entry.scoreTons} t
                </span>
              </div>
