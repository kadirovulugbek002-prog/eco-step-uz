...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Chiqindini qayta ishlang, Eco-Ballar to'plang va sayyorani toza saqlashga hissa qo'shing." },
    ],
  }),
  component: Dashboard,
});
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-8 p-6 md:p-10">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-[var(--gradient-hero)] p-8 text-primary-foreground shadow-[var(--shadow-eco)]">
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">Assalomu alaykum, Abdullo!</h1>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard
          label="Qayta ishlangan chiqindi"
          value={totalKg}
          unit="kg"
          icon={Recycle}
          trend="+12% bu hafta"
          accent="linear-gradient(135deg, oklch(0.62 0.17 152), oklch(0.72 0.18 162))"
        />
        <StatCard
          label="To'plangan Eco-Ballar"
          value={points.toLocaleString()}
          unit="ball"
          icon={Coins}
          trend="+48 bugun"
          accent="linear-gradient(135deg, oklch(0.72 0.18 165), oklch(0.78 0.16 145))"
        />
        <StatCard
          label="Kamaytirilgan CO₂"
          value={co2Saved}
          unit="kg"
          icon={Wind}
          trend="3 ta daraxtga teng"
          accent="linear-gradient(135deg, oklch(0.55 0.12 180), oklch(0.62 0.17 152))"
        />
      </div>
            ...
            {Array.from({ length: 7 }).map((_, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center gap-1">
                  <div
                    className="w-full rounded-md bg-[var(--gradient-primary)] transition-all hover:opacity-80"
                    style={{ height: `${h}px` }}
                  />
        ...
        <Card className="p-6 shadow-[var(--shadow-card)]">
          ...
          <div className="relative mx-auto my-6 grid h-44 w-44 place-items-center">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold">{weeklyProgress}</div>
              <div className="text-xs text-muted-foreground">/ {weeklyGoal} kg</div>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden shadow-[var(--shadow-card)]">
        <div className="flex items-center justify-between border-b border-border/60 p-6">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Sizning oxirgi topshiriqlaringiz</p>
          </div>
          <Button asChild variant="outline" size="sm">
            <Link to="/topshirish">Yangi topshirish</Link>
          </Button>
        </div>
        <div className="divide-y divide-border/60">
          {activities.slice(0, 6).map((a) => (
            <div key={a.id} className="flex items-center gap-4 p-4 transition-colors hover:bg-muted/40">
              <div className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--emerald)]/10 text-[var(--emerald)]">
                <Recycle className="h-5 w-5" />
              </div>
              <div className="flex-1">
                <div className="font-medium">{a.type}</div>
                <div className="text-xs text-muted-foreground">{a.date} · {a.weight} kg</div>
              </div>
